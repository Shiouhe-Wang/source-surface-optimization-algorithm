...
gs = gridspec.GridSpec(5, 4, hspace=0.365, wspace=0.275)  # 设置hspace为默认值，wspace为0
handles, labels = [], []
for i in range(5):
    for j in range(4):
        ...
        if i == 4 and j == 3:
            ...
            Brs = Bss(rssp, thetapsp, phiss, B_r, L) * rssp * rssp / (rpsp ** 2) * 1e+5
            Brs = np.interp(np.arange(len(Brs)), np.flatnonzero(~np.isnan(Brs)), Brs[~np.isnan(Brs)])
            c0 = np.max(np.abs(Brpsp)) / np.max(np.abs(Brs))
            mag = c0 * Brs
            ax = plt.subplot(gs[i, j])
            sc1=ax.scatter(times[Brpsp > 0], Brpsp[Brpsp > 0], color='#E64A19', s=0.7, label='Positive $B_r$')
            sc2=ax.scatter(times[Brpsp < 0], Brpsp[Brpsp < 0], color='#1976D2', s=0.7, label='Negative $B_r$')
            sc3=ax.scatter(times, mag, color='black', s=0.7, label='PFSS Simulation1')
            sc4=ax.scatter(times, mag2, color='#EF9163', s=0.7, label='PFSS Simulation2')
            if i == 0 and j == 0:
                handles.extend([sc1, sc2, sc3, sc4])
                labels.extend(['Positive $B_r$', 'Negative $B_r$', 'PFSS Simulation1', 'PFSS Simulation2'])
            #ax.set_xlabel('Date', fontsize=5)
            ax.set_ylabel(r'$\bf{B_r}$ / nT', fontsize=7,fontweight='bold')
            if j>=1:
                ax.set_ylabel('')
            ax.set_title(f'PSP Encounter' + sequence[k], fontsize=8,fontweight='bold')
